...
export class dropUnusedIndexes1629964808297 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    const indexItemsOnUserAndTimestamp = await queryRunner.manager.query('SHOW INDEX FROM `items` where `key_name` = "index_items_on_user_uuid_and_updated_at_and_created_at"')
    ...
    const indexItemsOnUpdatedAt = await queryRunner.manager.query('SHOW INDEX FROM `items` where `key_name` = "index_items_on_updated_at"')
